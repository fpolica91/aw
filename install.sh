#!/usr/bin/env bash
# aw CLI installer — downloads a prebuilt binary, no clone or Go toolchain needed.
#   curl -fsSL https://raw.githubusercontent.com/fpolica91/aw/main/install.sh | bash
set -euo pipefail

REPO="fpolica91/aw"
BINARY="aw"
INSTALL_DIR="${AW_INSTALL_DIR:-/usr/local/bin}"
...
if curl -fsSL "${base}/checksums.txt" -o "${tmp}/checksums.txt" 2>/dev/null; then
  ...
  if [ -n "$want" ]; then
    if command -v shasum >/dev/null 2>&1; then got="$(shasum -a 256 "${tmp}/aw" | awk '{print $1}')";
    else got="$(sha256sum "${tmp}/aw" | awk '{print $1}')"; fi
...
if [ -w "$INSTALL_DIR" ]; then
  mv "${tmp}/aw" "${INSTALL_DIR}/${BINARY}"
else
  echo "Installing to ${INSTALL_DIR} (requires sudo)…"
  sudo mv "${tmp}/aw" "${INSTALL_DIR}/${BINARY}"
fi

echo "✓ aw installed → ${INSTALL_DIR}/${BINARY}"
